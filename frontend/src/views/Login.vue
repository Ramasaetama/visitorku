  ...
  <div class="w-full h-full flex flex-col flex-1">
    ...
    <OnboardingLayout v-else :current-step="0" :total-steps="0" :show-progress="false">
      <div class="w-full h-full flex flex-col">
        ...
        <div class="flex-1">
          <h2 class="text-2xl lg:text-xl font-medium text-gray-900 mt-49 mb-0 font-poppins">
          ...
          <form @submit.prevent="handleLogin" class="space-y-6 form-custom-label">
            <Input
              v-model="form.email"
              label="Alamat Email"
              type="email"
            ...
            <div class="flex justify-end">
              <button 
                ...
                class="text-body-3 text-[#EE9D0F] hover:underline font-medium cursor-pointer bg-transparent border-none"
              >
                Lupa password
              </button>
            </div>
            ...
            <div class="flex justify-end">
              <button
                ...
                :class="[
                  'w-85 px-6 py-4 text-body-2 rounded-sm font-poppins font-semibold transition-colors duration-200 flex items-center justify-center gap-2 border-none',
          ...
          <p class="text-center text-body-3 text-gray-600 mt-8 font-poppins">
            Belum punya akun? 
            <router-link to="/register" class="text-[#EE9D0F] font-semibold hover:underline">
        ...
        <div class="mt-1">
          <div class="flex items-center justify-between text-sm text-gray-500 font-poppins py-4 border-t border-gray-100">
            <span>Visitorku Member of GMEDIA © 2025</span>
            <a href="#" class="flex items-center gap-2 text-[#EE9D0F] hover:underline">
              <span>Butuh Bantuan?</span>
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
              <span class="font-semibold text-[#EE9D0F]">Hubungi Support</span>
            </a>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showForgotPassword" class="fixed inset-0 z-[9999] flex items-center justify-center">
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (token) {
      // ⚠️ PERHATIAN: Di sini kamu nge-save token ke sessionStorage
      // Pastikan axios kamu (di src/services/api.js) ngambil token dari sessionStorage juga.
      // Kalau axios-nya ngambil dari localStorage, ini yang bikin error 401 Unauthorized!
...
/* CSS Khusus untuk memanipulasi label di dalam form agar TIDAK BOLD dan tanda bintang MERAH */
.form-custom-label :deep(label) {
  font-weight: 400 !important; /* Hilangkan bold */
  color: #374151; /* Warna abu-abu gelap */
}

/* Mengubah warna bintang (*) jadi merah. Berlaku kalau bintangnya di dalam span atau tag text-red-500 */
.form-custom-label :deep(label .text-red-500),
.form-custom-label :deep(label span) {
